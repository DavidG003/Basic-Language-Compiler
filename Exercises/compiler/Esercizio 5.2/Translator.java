    ...
    public void bexpr(int jump_label){
        ...
        switch (look.tag) {
            ...
            case '!':
                ...
                break;
            case Tag.AND:
            //ci sono due condizioni generate da 2 bexpr
            //se è falsa la prima condizione non controllo la seconda
                int lazy_and = code.newLabel(); 
                int second_cond = code.newLabel();               
                match(Tag.AND);
                bexpr(second_cond);
                //se è falsa salta il controllo della seconda
                code.emit(OpCode.GOto, lazy_and);
                code.emitLabel(second_cond);
                bexpr(jump_label);
                code.emitLabel(lazy_and);
                break;
            case Tag.OR:
                match(Tag.OR);

             
                //prima di aver controllato l'altra condizione salto direttamente a dove si trovava il caso vero del if
                //in questo modo il codice della seconda condizione verrà saltato nel caso prima condizione vera
                //se è falsa invece controlla la seconda condizione
                bexpr(jump_label);
                bexpr(jump_label);
                break;
            default:

...
public class Translator {
    ...
    public void stat() {
        switch (look.tag) {
            ...
            case Tag.IF:
                ...
                match(Tag.IF);
                match('(');
                bexpr(if_jump); //visto che inverto la condizione posso passare l'istruzione a cui deve passare
                match(')');
                
                //code.emit(OpCode.GOto, if_jump); non salto perche ho invertito la condizione nel if
                //code.emitLabel(if_continue); non serve visto che non è usato
                stat(); 
    ...
    public void statfor() {
        ...
        //int jump = code.newLabel(); non serve il label aggiuntivo           
        ...
        switch (look.tag) {
            case Tag.ID:
                ...
                bexpr(end_for); //salto a fine ciclo se la condizione invertita è vera
                match(')');
                match(Tag.DO);
                
                //code.emit(OpCode.GOto, end_for); non salto a fine for visto che la condizione invertita è falsa   
                //code.emitLabel(jump);
                stat();
    ...
    public void statif(int if_jump) {
        switch (look.tag) {
            case Tag.ELSE:
                ...
                code.emit(OpCode.GOto, end_else);// se la condizione invertita è falsa nel if allora devo salatare l'else
                code.emitLabel(if_jump); // se la condizione inveritata è vera nel if allora eseguo l'else
                match(Tag.ELSE);
                stat();
                match(Tag.END);
    ...
    public void bexpr(int jump_label){
        ...
        switch (look.tag) {
            case Tag.RELOP:
                ...
                expr();
                expr();
                /*inverto le condizioni per non generare un goto in più */
                switch (cond) {
                    case "<":
